...
test('POST /v1/moderation/bot-detection/games/:gameId/analyze: successful analysis as moderator', async () => {
  ...
  try {
    ...
    const res = await harness.json('POST', `/v1/moderation/bot-detection/games/${gameId}/analyze`, {
      token: mod.token,
    });
    ...
    // Follow-up: verify reports were stored and accessible via GET endpoint
    const listRes = await harness.json('GET', `/v1/moderation/bot-detection/players/${white.userId}/games`, {
      token: mod.token,
    });
...
test('POST /v1/moderation/bot-detection/games/:gameId/analyze: auth and role checks', async () => {
  const harness = await startHarness();
  try {
    ...
    // Plain user -> 403
    const resUser = await harness.json('POST', `/v1/moderation/bot-detection/games/${gameId}/analyze`, {
      token: user.token,
    });
    assert.equal(resUser.status, 403);

    // Unauthenticated -> 401
    const resAnon = await harness.json('POST', `/v1/moderation/bot-detection/games/${gameId}/analyze`);
...
test('POST /v1/moderation/bot-detection/games/:gameId/analyze: error cases (404, 422)', async () => {
  const harness = await startHarness();
  try {
    const mod = await harness.makeUser('mod', ['moderator']);
    ...
    // Unknown gameId -> 404
    const res404 = await harness.json('POST', `/v1/moderation/bot-detection/games/${unknownGameId}/analyze`, {
      token: mod.token,
    });
    assert.equal(res404.status, 404);

    // Malformed gameId -> 422
    const res422Id = await harness.json('POST', '/v1/moderation/bot-detection/games/not-a-uuid/analyze', {
      token: mod.token,
    });
