...
| Move explainer | Yes | Yes | N/A | No | `POST /v1/ai/move-explanation` (ADR-0115): authenticated, per-user rate limited, engine-grounded on the ADR-0113 analysis pool rather than a second one, with server-owned token/latency/failover ceilings that no request field can reach. The capability reports off unless both an AI provider and an engine are configured. No UI consumes it yet — that is M15 Inc 4. |
