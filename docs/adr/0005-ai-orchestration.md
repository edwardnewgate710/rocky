...
M7 ships two real HTTP adapters:

1. **`OpenAiCompatibleAdapter`** — a single adapter that covers OpenAI, DeepSeek, OpenRouter, and Ollama via configurable `baseUrl`. Uses the global `fetch` (Node 22+), no SDK dependency.

2. **`AnthropicAdapter`** — uses the Anthropic Messages API with `x-api-key` header and `anthropic-version` header.

Both adapters implement the `AiProvider` interface and are env-gated for integration tests (skip without API keys, exactly like the Postgres-gated persistence tests).

## Engine Grounding

The `engineResultsToGrounding()` function bridges `@chess-platform/engine` analysis results into `EngineGrounding` for LLM prompts. This satisfies the acceptance criterion: "grounded move-explanation cited against engine eval."

## Voting Deferral

Voting/ensemble is explicitly deferred to M8 (AI features). Ensemble behavior belongs with concrete AI feature use cases, not the orchestration framework.
