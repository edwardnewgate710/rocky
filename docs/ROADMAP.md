...
- **`ApiServer.listen` registers no `'error'` handler, so a failed bind hangs and raises an uncaught event.** `packages/api/src/server.ts` resolves its promise from the `listening` callback only. A bind that fails asynchronously (`EADDRINUSE`, `EMFILE`) therefore leaves the promise pending forever and, with no `'error'` listener on the `http.Server`, is re-raised as an uncaught exception. Found while investigating ADR-0140 and **left unfixed**: it is production code, it was not shown to cause either observed signature, and the increment that found it was scoped to the test harness. It is a candidate cause for the still-open signature B above.
