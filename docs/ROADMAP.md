...
- **`startHarness` occasionally fails a whole test file, and one signature is still unexplained (observed in M15 Increments 23 and 24).** `server.listen(0)` sometimes draws a port undici refuses, which surfaces as `TypeError: fetch failed` / `Error: bad port` with a stack pointing at the harness's first request — seen in `community-api`, `achievements-api` and `coach-route`, each passing in isolation. A second, distinct symptom has now appeared twice: a file marked failed with `'test failed'`, no assertion, no stack and none of its own tests reported — `move-explanation-route` in Increment 23 and `seek-concurrency` in Increment 24, both passing in isolation. **The two are deliberately not filed as one cause**, because the port flake announces itself with a stack and the second produces none. Both want a look at the harness's port acquisition in an increment of its own.
