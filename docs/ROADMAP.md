...
- **Clicking a search mode discards text typed since the page loaded (found in M15 Increment 24, not fixed).** `createModeInput` in `packages/web/src/app/search-mount.ts` closes over the query captured when the route mounted, so `navigateToSearchMode` navigates with the old query and the remount resets the input to match. Type a new term into the header field, click **Semantic** without pressing enter, and the typed text is gone. Pre-existing — the closure predates Increment 24 and is untouched by it — and found by the adversarial review of the Increment 24 branch, while reviewing the capability gate wrapped around the same control. Recorded rather than folded in because it is a different defect from capability parity with its own tests to write. The fix is to read the live input value at change time rather than closing over the mount-time one.
