...
**Verified.** Eleven regression tests in `reused-database.integration.test.ts` pin the mechanism, each
...
**184 / 184 / 184, zero failures**, after which the database holds no test rows at all — only the
three migration-seeded bot accounts — with zero leaked disposable databases and zero lingering
backends. Falsification killed **15 of 17** mutations; both survivors are named in the PR.
