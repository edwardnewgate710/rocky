...
_Last updated: 2026-07-16 — M12 inc 3: rate limiting for sensitive auth endpoints (ADR-0013). **M1–M8 complete, M12 inc 1–3 complete, M14 increments 1–4 complete (M14 overall still in progress).** Prior: M14 increment 4 (Kubernetes Helm chart). **M7, M8, M14 inc 1–4 complete.** Prior: Review #03 fixes applied:
...
| **M9** 🚧 | `@chess-platform/tournament` | **Increment 1 (pure domain):** tournament aggregate with a `registration → running → finished` state machine, a `PairingStrategy` port, `RoundRobinPairing` (circle-method/Berger schedule — every pair once, one bye per player for odd N, balanced colors), and Sonneborn-Berger standings. No persistence/API/realtime yet (later increments). Swiss + Arena are follow-ups on the same port; ADR-0014 Accepted | 20/20 |
...
**Whole-repo total: ~785 tests, 0 failures, across 11 packages + the gateway service** (skips: 2 Postgres-gated + 18 API-key-gated). Strict TS, zero errors, lint clean. CI active — 6 jobs: build+typecheck+test on Node 22/24, Postgres integration, M6 Playwright + Lighthouse acceptance, helm lint + kubeconform, gateway service (build + Redis integration).
